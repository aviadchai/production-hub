...
export function ProjectReferences({ projectId }: { projectId: string }) {
  ...
  const addRef = () => {
    ...
    setRefs((prev) => [
      ...prev,
      { id: String(Date.now()), project_id: projectId, category: form.category, title: form.title, image_url: null, notes: form.notes || null },
    ])
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {items.map((ref) => (
                  <div key={ref.id} className="group relative rounded-lg border border-border bg-secondary/30 p-3 space-y-1.5 hover:bg-secondary/50 transition-colors">
                    ...
                    <button onClick={() => removeRef(ref.id)} className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity h-4 w-4 rounded bg-background/80 flex items-center justify-center">
                      <X className="h-2.5 w-2.5 text-muted-foreground" />
                    </button>
                  </div>
                ))}
                <button onClick={() => { setForm(f => ({ ...f, category: cat })); setOpen(true) }} className="aspect-auto rounded-lg border border-dashed border-border/50 p-3 flex items-center justify-center text-muted-foreground/40 hover:text-muted-foreground hover:border-border transition-colors min-h-[80px]">
                  <Plus className="h-4 w-4" />
                </button>
              </div>
            </div>
          ))}
          <button onClick={() => setOpen(true)} className="flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground transition-colors">
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <select value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} className="w-full h-8 rounded-md border border-input bg-background px-3 text-sm">
                {referenceCategories.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Title</Label>
              <Input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="e.g. Dan — Lead Actor" className="h-8 text-sm" autoFocus />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Notes (optional)</Label>
              <Textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} placeholder="Any notes..." className="text-sm resize-none min-h-[60px]" />
            </div>
          </div>
